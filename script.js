/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   SYNAPTIC SPARK EFFECT (red palette)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
(function initSparks() {
  ...
  const COLORS = ['#ef4444', '#f97316', '#fca5a5'];
  ...
  function spawnEdge() {
    const candidates = [...document.querySelectorAll('section, .cancer-card, .organ-card, .source-mockup, .sec-source-card, .source-type-card, .lobe-table-wrap, .cell-player')];
    ...
    if (edge === 0)      { sx = rect.left + Math.random() * rect.width;  sy = rect.top;    mainAxis = 'x'; }
    else if (edge === 1) { sx = rect.left + Math.random() * rect.width;  sy = rect.bottom; mainAxis = 'x'; }
    else if (edge === 2) { sx = rect.left;  sy = rect.top + Math.random() * rect.height;   mainAxis = 'y'; }
    else                 { sx = rect.right; sy = rect.top + Math.random() * rect.height;   mainAxis = 'y'; }

    const dir      = Math.random() > 0.5 ? 1 : -1;
    const stepMain = dir * (5 + Math.random() * 9);
    ...
    for (let i = 0; i < segments; i++) {
      ...
      const nx   = mainAxis === 'x' ? last.x + stepMain + (Math.random() - 0.5) * 3 : last.x + jit;
      const ny   = mainAxis === 'y' ? last.y + stepMain + (Math.random() - 0.5) * 3 : last.y + jit;
  ...
  window.addEventListener('keydown', e => {
    ...
    for (let i = 0; i < count; i++) Math.random() > 0.4 ? spawnEdge() : spawnAmbient();
  });
...
(function initCellField() {
  ...
  const cells = [
    { s: 480, x:  5, y: 10, d: 24, del:   0, c: '205,92,92'  },
    { s: 300, x: 72, y: 55, d: 18, del:  -6, c: '233,150,122' },
    { s: 370, x: 42, y: 78, d: 28, del: -12, c: '250,200,200' },
    { s: 195, x: 88, y:  8, d: 16, del:  -3, c: '205,92,92'  },
    { s: 540, x: 26, y: 42, d: 32, del:  -9, c: '253,230,230' },
    { s: 240, x: 62, y: 22, d: 20, del: -15, c: '233,150,122' },
    { s: 155, x: 14, y: 87, d: 14, del:  -2, c: '205,92,92'  },
  ];
...
const POSITIONS = [
  ...
  { x: 44, y: 55 }, { x: 60, y: 20 }, { x: 74, y: 65 }, { x: 88, y: 30 },
];
...
class CellPlayer {
  ...
  _build() {
    const bgHTML = ORGANELLE_CONFIGS.map((cfg, i) => {
      ...
      return `<span class="organelle ${cfg.cls}" style="left:${pos.x}%;top:${pos.y}%;--dur:${cfg.dur}s;--delay:${(i*0.85).toFixed(2)}s;--dx1:${cfg.dx1}px;--dy1:${cfg.dy1}px;--dx2:${cfg.dx2}px;--dy2:${cfg.dy2}px;--dx3:${cfg.dx3}px;--dy3:${cfg.dy3}px;"></span>`;
    }).join('');

    const codonsHTML = Array.from({ length: CODON_COUNT }, (_, i) => {
      const isTriplet = i % 3 === 2;
      return `<span class="codon-tick" style="height:${isTriplet?10:5}px;opacity:${isTriplet?0.45:0.22}"></span>`;
    }).join('');
    ...
    this.el.innerHTML = `
      ...
      <div class="cell-controls">
        <button class="nucleus-btn" aria-label="Play" ${this.hasSrc ? '' : 'disabled'}>${ICONS.play}</button>
        <div class="mrna-wrap">
          ${label ? `<span class="mrna-label">${label}</span>` : ''}
          <div class="mrna-track">
            <div class="mrna-codons">${codonsHTML}</div>
            <div class="mrna-fill"></div>
            <div class="ribosome"><div class="rib-large"></div><div class="rib-small"></div><div class="rib-tunnel"></div></div>
          </div>
        </div>
  ...
  _play() {
    document.querySelectorAll('.cell-player').forEach(p => { if (p !== this.el && p._cp) p._cp._pause(); });
    this.audio.play();
    this.playing = true;
...
document.querySelectorAll('.cell-player').forEach(el => { el._cp = new CellPlayer(el); });
